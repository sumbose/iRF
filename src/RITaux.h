...
// We assume elements of A and B are sorted in ascending order
...
void CreateHt(RaggedArray &x, const int L, int **Ht) {
    mt19937_64 mt(INT_MAX * runif(1)[0]);
    ...
    // Vector perm_i=i
    IntegerVector perm = Rcpp::seq(0, n-1);
...
// Computes P1(L,intersection,H) =
// 1 / L{#l st H[l][k]=H[l][k'] for all k,k' in intersection}
// Computes P2(L,intersection,H) =
// {(n+1)/n} * {1/(average over l(min(H[l][k]))) - 1/(n+1)}
double PrevEst(const vector<int> &intersection, int **Ht, const int L,
        const double n_plus_1_over_n, const double recip_n_plus_1) {

    // If the interaction has size 1, return 1
    ...
    return n_plus_1_over_n
        * (double(L)/P2-recip_n_plus_1)
        * double(P1)/double(L);
}

// Given a set of interactions (integer vectors) copmutes their prevalence
// using the minwise hash matrix
// Returns a vector of prevalences
vector<double> PrevEst_inter(const set<vector<int> > &interactions, int **Ht,
        const int L, const double n_plus_1_over_n,
...
// Given a set of interactions (integer vectors), this produces
// a set with 1 added to each entry
// Note function gives an error when we pass &interactions instead
// and try to modify interactions
