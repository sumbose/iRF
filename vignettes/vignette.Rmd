...
paper: https://arxiv.org/abs/1810.07287.
    ...
    + `int` indicates the interaction, with $+$ and $-$ describing whether the
      ...
      instance, the interaction `1+_2+_3+` describes an interaction that is
      defined by high levels of $1, 2,$ and $3$. 

    + `prevalence.diff` indicates the difference in prevalence between class-$0$ and
      class-$1$ leaf nodes, `sta.diff` indicates the proportion of times this value
      is greater than 0 across bootstrap replicates. 

    + `independence` indicates the difference between the prevalence of an
      interaction and the expected prevalence of an interaction if features were
      selected independently, scaled by the prevalence of the interaction,
      `sta.independence` indicates the proportion of times this value is greater
      than 0 across bootstrap replicates. 

    + `precision` indicates the difference between the proportion of class-1
      observations in leaf nodes containing the interaction and the proportion of
      class-1 leaf nodes in leaf nodes containing subsets of the interaction,
      `sta.precision` indicates the proportion of times this value is greater than 0
